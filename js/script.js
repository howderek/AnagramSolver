...
function handleInput() {
    ...
    if (document.getElementById('bigBar').value !== oldWord && document.getElementById('bigBar').value.length > 2 && !(document.getElementById('bigBar').value.match(/!|-/g))) {
        ...
        oldWord = '';
        document.getElementById('results').innerHTML = panelHTML.replace('TITLE', 'AnagramSolver').replace('CONTENT', 'Great for solving crosswords and unscrambling puzzles!<br><br>Type a word to get anagrams and caesar shifts (Example: <a href="index.html?q=tesla">tesla</a>), or type with dashes (Example: <a href="index.html?q=d---k">d---k</a>) to find possible fill in the blank solutions.<br><br>Type an exclalmation mark to run commands (Example: <a href="index.html?q=!random">!random</a>)<br><br> It is written entirely in JavaScript, uses <a href="http://getbootstrap.com">Twitter Bootstrap</a> and is responsive!<br><br>The current wordlist is ' + words.length.toLocaleString()  + ' words long, and your benchmark completed in ' + elapsed + 'ms.<br><br>The word of the day is <em>' + wordOfTheDay + '</em><br><br>AnagramSolver &copy 2013 <a href="http://howderek.com">Derek Howard</a>, distributed under the MIT license. <a href="http://github.com/howderek/">GitHub</a>');
    }
}
